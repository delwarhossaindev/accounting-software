...
<style>
    * { box-sizing: border-box; }
    ...
    .invoice-title { background: #e9ecef; border: 1px solid #333; font-weight: 700; padding: 5px 15px; font-size: 13px; text-align: center; display: inline-block; margin: 10px auto; }
    ...
    .items-table .serial-no { font-size: 9px; color: #444; margin-top: 2px; }
...
<div style="text-align: center; margin: 10px 0;">
    <span class="invoice-title">{{ $invoice->type === 'sales' ? 'Invoice / Bill' : 'Purchase Bill' }}</span>
</div>

<table class="details-table">
    <tr>
        <td class="label">Invoice No</td><td class="value">: <strong>{{ $invoice->invoice_no }}</strong></td>
        <td class="label">Date</td><td class="value">: {{ $invoice->date->format('d-m-Y') }}</td>
    </tr>
    <tr>
        <td class="label">{{ $invoice->type === 'sales' ? 'Customer Name' : 'Supplier Name' }}</td>
        <td class="value">: <strong>{{ $invoice->type === 'sales' ? ($invoice->customer->name ?? '-') : ($invoice->supplier->name ?? '-') }}</strong></td>
        <td class="label">Branch</td>
        <td class="value">: {{ $invoice->branch?->name ?? '-' }}</td>
    </tr>
    <tr>
        <td class="label">Address</td>
        <td class="value">: {{ $invoice->type === 'sales' ? ($invoice->customer->address ?? '') : ($invoice->supplier->address ?? '') }}</td>
        <td class="label">P.O.No</td>
        <td class="value">: {{ $invoice->po_no ?? '' }}</td>
    </tr>
    <tr>
        <td class="label">Mobile</td>
        <td class="value">: {{ $invoice->type === 'sales' ? ($invoice->customer->phone ?? '') : ($invoice->supplier->phone ?? '') }}</td>
        <td class="label">Req. No</td>
        <td class="value">: {{ $invoice->req_no ?? '' }}</td>
    </tr>
    <tr>
        <td class="label">Contact Name</td>
        <td class="value">: </td>
        <td class="label">Sold By</td>
        <td class="value">: {{ $invoice->sold_by ?? '' }}</td>
    </tr>
    <tr>
        <td class="label">E-mail</td>
        <td class="value">: {{ $invoice->type === 'sales' ? ($invoice->customer->email ?? '') : ($invoice->supplier->email ?? '') }}</td>
...
<table class="items-table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                @if($item->serial_no)
                    <div class="serial-no">SN : {{ $item->serial_no }}</div>
                @endif
            </td>
    ...
    <tfoot>
        <tr class="totals-row">
            <td colspan="3" rowspan="{{ ($invoice->tax > 0 ? 1 : 0) + ($invoice->discount > 0 ? 1 : 0) + 2 }}" class="words-row">
                IN WORDS : {{ strtoupper(numberToWords($invoice->total)) }} TAKA ONLY.
            </td>
        ...
        <tr class="totals-row">
            <td colspan="2" class="right">Due :</td>
            <td class="right">{{ number_format($invoice->due, 2) }}</td>
        </tr>
    </tfoot>
</table>
...
<table class="signature-section">
    <tr>
        <td width="45%">
            ...
            <span style="font-size: 9px;">(Received the above goods in good condition)</span>
        </td>
        <td width="10%"></td>
        <td width="45%" style="text-align: right;">
            <div class="signature-line" style="margin-left: auto; display: inline-block; width: 250px;">......................................................</div>
            <br><strong>Authorized's Signature</strong>
        </td>
    </tr>
</table>
